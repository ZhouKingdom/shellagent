...
def generate_task_toml(task_meta: dict, *, source: str) -> str:
    ...
    name = task_meta.get("name", "unknown") or "unknown"
    scenario = task_meta.get("scenario", "") or ""
    domain = task_meta.get("domain", "") or ""
    skill_type = task_meta.get("skill_type", "") or ""
    primitive_skills = task_meta.get("primitive_skills", []) or []
    task_complexity = task_meta.get("task_complexity", "") or ""
    command_complexity = task_meta.get("command_complexity", "") or ""
    language = task_meta.get("language", "") or ""
    base_image = task_meta.get("base_image", "") or ""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "base once, e.g. shellagent/base-intricate:1.0)."
        ),
    )
    parser.add_argument(
        "--workers",
