...
# ---- Load shellagent secrets (GEMINI_API_KEY, Docker Hub creds, etc.) ---------
# When this script is launched via `sbatch` rather than `bash` inside an
# already-`source ~/.shellagent_secrets`'d interactive shell, the sbatch job
...
if [[ -f "$HOME/.shellagent_secrets" ]]; then
  # shellcheck disable=SC1091
  source "$HOME/.shellagent_secrets"
fi
: "${GEMINI_API_KEY:?GEMINI_API_KEY not set; ensure ~/.shellagent_secrets exists and is sourced}"
