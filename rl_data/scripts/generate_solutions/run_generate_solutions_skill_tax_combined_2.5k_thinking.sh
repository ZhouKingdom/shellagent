...
#SBATCH --output=/gpfs/scrubbed/osey/shellagent/logs/gen_sol_stx_comb_2.5k_thinking_%j.out
...
# ║  --export=ALL propagates them, OR placed in $SHELLAGENT_SECRETS_FILE which ║
...
# When the user remembered to `source ~/.shellagent_secrets` before sbatch and
...
_SHELLAGENT_SECRETS_FILE="${SHELLAGENT_SECRETS_FILE:-$HOME/.shellagent_secrets}"
if [[ -r "$_SHELLAGENT_SECRETS_FILE" ]]; then
  echo "=== Sourcing secrets from $_SHELLAGENT_SECRETS_FILE ==="
  # shellcheck disable=SC1090
  set +u
  source "$_SHELLAGENT_SECRETS_FILE"
  set -u
fi
...
# `sbatch` / `salloc` was invoked from — that IS the shellagent root in our
...
# you `cd` to shellagent. Blindly trusting it would break. We therefore only
...
if [[ -n "${SHELLAGENT_PROJECT_ROOT:-}" ]] && [[ -r "$SHELLAGENT_PROJECT_ROOT/rl_data/scripts/comparison/_vllm_local.sh" ]]; then
  PROJECT_ROOT="$SHELLAGENT_PROJECT_ROOT"
...
if [[ ! -r "$PROJECT_ROOT/rl_data/scripts/comparison/_vllm_local.sh" ]]; then
  ...
  echo "       * sbatch/salloc: launch from the shellagent repo root, OR unset SLURM_SUBMIT_DIR and run again from shellagent," >&2
  echo "         OR set SHELLAGENT_PROJECT_ROOT=/path/to/shellagent explicitly (advanced)." >&2
  exit 2
fi
...
export APPTAINER_DOCKER_USERNAME="${APPTAINER_DOCKER_USERNAME:?Set APPTAINER_DOCKER_USERNAME before running (or put it in \$SHELLAGENT_SECRETS_FILE)}"
export APPTAINER_DOCKER_PASSWORD="${APPTAINER_DOCKER_PASSWORD:?Set APPTAINER_DOCKER_PASSWORD before running (or put it in \$SHELLAGENT_SECRETS_FILE)}"
