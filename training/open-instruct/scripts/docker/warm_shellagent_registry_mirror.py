#!/usr/bin/env python3
"""Warm a Docker Hub pull-through registry cache from a ShellAgent HF dataset.

The ShellAgent/SWERL datasets contain task directories with ``image.txt`` files,
usually packaged as ``task-data.tar.gz``. ShellAgent rows also carry per-sample
...
Examples:
    uv run python scripts/docker/warm_shellagent_registry_mirror.py \\
        --dataset hamishivi/swerl-shellagent-15k \\
        --mirror jupiter-cs-aus-193.reviz.ai2.in:5000

    uv run python scripts/docker/warm_shellagent_registry_mirror.py \\
        --tool-configs '{"task_data_hf_repo":"hamishivi/swerl-shellagent-15k","image":"python:3.12-slim"}' \\
        --mirror jupiter-cs-aus-193.reviz.ai2.in:5000
...
the mirror by running pulls like:
    docker pull jupiter-cs-aus-193.reviz.ai2.in:5000/hamishi740/swerl-shellagent-v3:tag
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset",
        default=None,
        help="HF dataset repo in ShellAgent/SWERL task-data format. Defaults to task_data_hf_repo from --tool-configs.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            '\'{"task_data_hf_repo":"hamishivi/swerl-shellagent-15k","image":"python:3.12-slim"}\'. '
