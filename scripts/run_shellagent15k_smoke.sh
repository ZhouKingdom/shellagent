...
# Smoke-test the converted ShellAgent-15K-Harbor dataset with Terminus-2 + Claude on
...
#
# Usage:
#   bash scripts/run_shellagent15k_smoke.sh
#   N_LEGACY=3 N_INTRICATE=3 bash scripts/run_shellagent15k_smoke.sh
#
...
#   JOB_NAME       - job name / resume key (default: shellagent15k_smoke_claude)
#   FULL_DATASET   - converted Harbor dataset path

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

# ---------- Load keys (~/.shellagent_secrets then .harbor.env; latter wins) ----------
set -a
# shellcheck disable=SC1090,SC1091
[ -f "$HOME/.shellagent_secrets" ] && source "$HOME/.shellagent_secrets"
...
JOB_NAME="${JOB_NAME:-shellagent15k_smoke_claude}"
FULL_DATASET="${FULL_DATASET:-rl_data/output/tasks_skill_tax_combined_20260506_legacy10k_new5k_harbor}"
TEST_DATASET="rl_data/output/shellagent15k_smoke_${N_LEGACY}l_${N_INTRICATE}i_harbor"
