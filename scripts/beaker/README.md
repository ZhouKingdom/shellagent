...
```
/weka/oe-adapt-default/$USER/shellagent-eval/<job-name>/jobs/<job-name>/
```
...
3. `git clone` the shellagent repo at the SHA you launched from (the wrapper reads
...
| `--results-dir DIR` | `/weka/oe-adapt-default/$USER/shellagent-eval/<job-name>` | Destination on weka. |
| `--cluster CLUSTER` | `ai2/saturn` | |
| `--budget BUDGET` | `ai2/oe-adapt` | |
| `--priority PRI` | `high` | |
| `--workspace WS` | `$BEAKER_WORKSPACE` or `ai2/shellagent` | |
...
- Aggregate: `/weka/.../shellagent-eval/<job-name>/jobs/<job-name>/result.json` —
  reward distribution + exception stats.
- Stats summary: `/weka/.../shellagent-eval/<job-name>/jobs/<job-name>/stats.txt` —
  mean reward, standard deviation, SEM, and pass@k.
- Metrics JSON: `/weka/.../shellagent-eval/<job-name>/metrics.json` and
  `/weka/.../shellagent-eval/<job-name>/jobs/<job-name>/metrics.json` — structured
  mean reward, standard deviation, SEM, run scores, pass@k, and per-task stats.
- Per-trial: `/weka/.../shellagent-eval/<job-name>/jobs/<job-name>/<task>__<rand>/`
